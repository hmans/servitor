...
import { subscribeWorktreeChanges } from '$lib/server/worktree-watcher';
...
export function GET({ params, request }) {
	...
	return createSSEResponse(request, (send) => {
		...
		const unsubManager = subscribe(managerKey, (event) => {
			send(event.type, event);
		});

		const unsubWatcher = subscribeWorktreeChanges(ws.worktreePath, () => {
			send('worktree_changed', {});
		});
