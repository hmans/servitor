...
describe('parseClaudeEvent', () => {
  ...
  it('parses EnterPlanMode as enter_plan', () => {
    ...
    expect(events).toEqual([{ type: 'enter_plan', toolUseId: 'tool-2', sessionId: SESSION }]);
  });
  ...
  it('handles assistant event with empty content', () => {
    const events = parseClaudeEvent({ type: 'assistant', message: { content: [] } }, SESSION);
    expect(events).toEqual([]);
  });
  ...
  describe('result events', () => {
    ...
    it('falls back to passed sessionId when session_id missing', () => {
      ...
      expect(events).toEqual([{ type: 'message_complete', text: 'done', sessionId: SESSION }]);
    });
  });
});
